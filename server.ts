...
function getFallbackReport(assessmentData: any) {
...
app.post('/api/analyze', async (req, res) => {
  ...
  try {
    if (process.env.GEMINI_API_KEY) {
      const promptText = `사용자 마음 진단 데이터:
...
- 사용자가 남긴 마음 메모: "${assessmentData.userNotes || '메모 없음'}"

위 임상적 지표를 종합하여 System Instructions의 규칙에 맞춰 위로와 해결책을 담은 JSON 리포트를 생성해 주세요.`;
      ...
      // Fallback
      console.warn('GEMINI_API_KEY not set. Using clinical fallback heuristic generator.');
